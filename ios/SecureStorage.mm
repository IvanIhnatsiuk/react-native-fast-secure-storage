...
CFStringRef _accessibleValue(NSString *accessible) {
    NSDictionary *list = @{
        ...
        @"AccessibleWhenPasscodeSetThisDeviceOnly": (__bridge id)kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly,
        @"AccessibleWhenUnlockedThisDeviceOnly": (__bridge id)kSecAttrAccessibleWhenUnlockedThisDeviceOnly,
        @"AccessibleAfterFirstUnlockThisDeviceOnly": (__bridge id)kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly,
...
void install(jsi::Runtime &runtime,
    ...
    auto getAllKeys = HOSTFN("getAllKeys", 0) {
        ...
        if (statusCode == noErr) {
            ...
            for (NSDictionary *item in array) {
                NSString *key = [[NSString alloc] initWithData:item[(id)kSecAttrAccount] encoding:NSUTF8StringEncoding];
                [keys addObject: key];
            }
    ...
    auto getAllItems = HOSTFN("getAllItems", 0){
        ...
        if (statusCode == noErr) {
            ...
            for (NSDictionary *item in array) {
                NSString *key = [[NSString alloc] initWithData:item[(id)kSecAttrAccount] encoding:NSUTF8StringEncoding];
                NSString *value = [[NSString alloc] initWithData:item[(id)kSecValueData] encoding:NSUTF8StringEncoding];
    ...
    auto hasValue = HOSTFN("hasValue", 1) {
        ...
        @try {
            ...
            CFTypeRef result = NULL;
            OSStatus status = SecItemCopyMatching((CFDictionaryRef)queryDictionary,
                                                  (CFTypeRef *)&result);
