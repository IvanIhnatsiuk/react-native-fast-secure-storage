    ...
    void install(jsi::Runtime &runtime,
        ...
        auto setItem = HOSTFN("setItem", 1) {
            ...
            try {
                ...
                auto errorStr = jsi::String::createFromUtf8(runtime, "secure storage could not set value, error code: " + std::string(e.what()));
                return { false};
            }
        ...
        auto setItems = HOSTFN("setItems", 1 ) {
           try {
               auto items = arguments[0].getObject(runtime).asArray(runtime).asArray(runtime);
               for(int i = 0; i < items.size(runtime); i++) {
                   auto item = items.getValueAtIndex(runtime, i).getObject(runtime);
                   auto key = item.getProperty(runtime, "key").asString(runtime).utf8(runtime);
                   auto value = item.getProperty(runtime, "value").asString(runtime).utf8(runtime);
                   _set(key.c_str(), value.c_str());
                   return jsi::Value(true);
               }} catch (exception &e){
               return jsi::Value(false);
